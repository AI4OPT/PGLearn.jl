...
function main_ref(config; export_ref=false)
    ...
    data = PGLearn.OPFData(make_basic_network(PowerModels.parse_file(case_file)); compute_clique_decomposition=get(config["sampler"], "compute_clique_decomposition", false))
